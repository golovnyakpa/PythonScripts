...
def key_expansion(key):
    ...
    # add key length to size = 4 * nk
    if len(key_symbols) < 4 * nk:
        for i in range(4 * nk - len(key_symbols)):
            key_symbols.append(0x01)
    # initialization of key_schedule
    key_schedule = [[] for _ in range(4)]
    for i in range(4):
        for j in range(nk):
            key_schedule[i].append(key_symbols[i + j*4])
    # filling in the key_schedule
    for col in range(nk, (nr + 1)*nb):
        temp = []    # previous (col - 1) column
        ...
        if col % nk == 0:
            ...
            for i in range(nk):
                row = temp[i] // 16
                column = temp[i] % 16
                temp[i] = sbox[column + 16 * row]
                result.append(temp[i] ^ temp_2[i] ^ rcon[i][col // nk - 1])
        else:
            for i in range(nk):
